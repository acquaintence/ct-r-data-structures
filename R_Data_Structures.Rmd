...
```{r}
my_vector  <- c(10,14,60,1,20304, -6)
sort(my_vector)
```
...
```{r}
student_grades <-  c(98,94,74,78,82.91)
student_names <-  c('John','Jenn','Jake','Joe','Jane')
...
student_class_grade <-  c(98,90,97)

# create a dataframe from these three vectoras, each vector is its own columns
...
```{r}
student_name <-  c('Chak','Khemmy')
student_grade_level <- c('Freshman','Freshman')
student_class_grade <-  c(69,96)
student_subject <- c('Math','Art')
...
matrix2 <- matrix(c(4,5,6,7,8,9), nrow <-  3)
...
my_factor <- factor(c('Blue','Green','Brown','Blue','Green','Grey'))
my_factor
```
...
```{r}
my_factor <- factor(c('Blue','Green','Brown','Blue','Green','Grey'),
          levels = c('Brown','Green','Grey','Blue')
)
sort(my_factor)
```
...
my_array <-  array(
  ...
  dim <- c(3,3,2),
  dimname <- list(c('ROW 1','ROW 2','ROW 3'),c('COL 1','COL 2','COL 3'),c('MATRIX 1', 'MATRIX 2'))
)
my_array
```
...
4.  Print the second column of both matrices.
    ...
    ```{r}
    print('Print 5:')
    print(my_array[2,2,2])

    print("Print the third row of the first matrix:")
    print(my_array[3,,1])

    print("Print the second column of both matrices:")
    print(my_array[,2,])
